...
\begin{document}
	\begin{tikzpicture}[domain=0:2, scale=5]		
		\filldraw[color=gray, opacity = 0.5] (0,1.2) -- (0.8,1.2)  -- (0,2)  ;		
		...
		%punkte mit beschr. 
		\node at (0.8,1.2)[circle,fill=red,inner sep=2pt]{};
		
		%curves
		\draw (2,0)--(0,2) node[above right] {$P(Q)$};
		\draw (0,1.2)--(2,1.2) node[right] {$MC_{NLD}$};
		\draw (0,2)--(2,2) node[right] {$MC_{DEU}$};
	\end{tikzpicture}
\end{document}