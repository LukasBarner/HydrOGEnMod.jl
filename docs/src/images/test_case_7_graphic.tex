...
\begin{document}
	\begin{tikzpicture}[domain=0:2, scale=5]		
		\filldraw[color=gray, opacity = 0.5] (0,0.45) -- (0.5,0.5) -- (1.5,0.5)  -- (0,2)  ;		
		...
		%punkte mit beschr. 
		\node at (1.5,0.5)[circle,fill=red,inner sep=2pt]{};
		\node at (0.5,0.5)[circle,fill=red,inner sep=2pt]{};
		
		%curves
		\draw (2,0)--(0,2) node[right] {$P(Q)$};
		\draw (0,0.5)--(2,0.5) node[right] {$MC_{FES}$};
		\draw (0,0.45)--(2,0.65) node[right] {$MC_{RES}$};
	\end{tikzpicture}
\end{document}