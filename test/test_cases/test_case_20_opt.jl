function test_case_20_opt(config)
    @testset "Test Case 20" begin
        ...
        data = get_HydrOGEnMod_data(; data = joinpath(@__DIR__, "test_20"))
        ...
        @testset "Results" begin
            ...
            @testset "Prices" begin
                ...
                @test isapprox(
                    value.(model[:prices])["DEU", "NH3", "Block 1", "OnlySeason", 2020],
                    0.00,
                    atol = config.testing_tol,
                )
                @test isapprox(
                ...
                @test isapprox(
                    value.(model[:prices])["DEU", "NH3", "Block 1", "OnlySeason", 2021],
                    1.50,
                    atol = config.testing_tol,
                )
                @test isapprox(
